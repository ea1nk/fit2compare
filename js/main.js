...
/* INITIALIZE PARSER */
var EasyFit = require("EasyFit").default;
...
function parseFitdata(data) {
  var response;
  easyFit.parse(data, function(error, output) {
    if (error) {
      console.log("Error!");
      response = { error: true };
    } else {
  ...
  records.forEach(record => {
    if (!now) {
      ...
      if (moment(record.timestamp).diff(now, "seconds") <= 2) {
        ...
        xlsData.push({tsmp:new Date(now.format()).getTime(), pwr:power, hr:hr, cad:cadence});
...
function exportJPEG(id) {
  var graphs = ["", "pwrGraph", "hrGraph", "cadenceGraph"];
...
var rows = [];
data.forEach(entry =>{
  rows.push({a:entry.tsmp, b:entry.pwr, c:entry.hr, d:entry.cad})
})
return rows;
}
